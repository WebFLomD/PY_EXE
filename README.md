...
#### Вариант А. Быстрая сборка (рекомендуется)
в консоле прописываем следующее:
```bash
PyInstaller --onefile --icon "img/icon.ico" --name "Test" main.py
```
После сборки появятся папки `build` и `dist`, а в папке `dist` будет готовый EXE-файл, внутри которого уже находятся все изображения!

#### Вариант Б. Сборка через .spec
в консоле прописываем следующее:
```bash
PyInstaller --onefile --add-data "img;img" --name "Test" main.py
```
...
### Примечание (17.03.2026)

Я заметил, что после выполнения команды `PyInstaller --onefile --add-data "img;img" --icon "img/icon.ico" --name "Test" main.py`, не обязательно редактировать `.spec` файл. 

**Почему так работает?**
- Параметр `--add-data` сам добавляет изображения в сборку
- Функция `resource_path()` правильно находит их во временной папке при запуске EXE

**Вариант А** полностью рабочий и не требует ручной правки .spec файла.<br>
**Вариант Б** нужен только для сложных проектов, где требуется тонкая настройка сборки (например, много разных файлов или специфические зависимости).
...
- [YouTube - Python в .EXE ► КАК?](https://www.youtube.com/live/CgCJwDjm_Cw)
